...
public interface {$TableName}Service {
    ...
    /*分页查询{$table_des}数据*/
    ArrayList<{$table_seg_[1]['Name']}Vo> query{$TableName}({$table_seg_['end']['Name']}QueryDto {$table_seg_['end']['name']}QueryDto);

    /*查询所有{$table_des}数据*/
    ArrayList<{$table_seg_[1]['Name']}Vo> query{$TableName}All({$table_seg_['end']['Name']}Dto {$table_seg_['end']['name']}Dto);
